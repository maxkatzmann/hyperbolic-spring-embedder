...
class Gnuplot {
 ...
 public:
  ...
  /// plot an equation supplied as a std::string y=f(x), write only the function
  /// f(x) not y= the independent variable has to be x binary operators: **
  /// exponentiation, * multiply, / divide, + add, - substract, % modulo unary
  /// operators: - minus, ! factorial elementary functions: rand(x), abs(x),
  /// sgn(x), ceil(x), floor(x), int(x), imag(x), real(x), arg(x),
  ///   sqrt(x), exp(x), log(x), log10(x), sin(x), cos(x), tan(x), asin(x),
  ///   acos(x), atan(x), atan2(y,x), sinh(x), cosh(x), tanh(x), asinh(x),
  ///   acosh(x), atanh(x)
  /// special functions: erf(x), erfc(x), inverf(x), gamma(x), igamma(a,x),
  /// lgamma(x), ibeta(p,q,x),
...
std::string Gnuplot::terminal_std = "png";
#endif
...
Gnuplot &Gnuplot::savetops(const std::string &filename) {
  ...
  cmdstr << "set output \"|ps2pdf - " << filename << "\"";
  cmd(cmdstr.str());
